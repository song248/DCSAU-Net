    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'valid']:
            ...
            for inputs,labels,image_id in dataloaders[phase]:      
                ...
                score = accuracy_metric(outputs,labels) # IoU
                
                smooth=1
                dinputs = torch.sigmoid(outputs)
                intersection = (dinputs * labels).sum()    
                dice = (2.*intersection + smooth)/(dinputs.sum() + labels.sum() + smooth) 
            ...
            print('{} Loss: {:.4f} IoU: {:.4f} Dice: {:.4f}'.format(
