...
class CombinedLoss(nn.Module):
    def __init__(self, dice_weight=0.5, ssim_weight=0.5):
        ...
        self.dice_loss = DiceLoss_binary()  # 기존의 DiceLoss_binary 사용
        self.ssim_loss = SSIMLoss()
        self.dice_weight = dice_weight
        self.ssim_weight = ssim_weight

    def forward(self, outputs, targets):
        dice_loss_value = self.dice_loss(outputs, targets)
        ssim_loss_value = self.ssim_loss(outputs, targets)
        return self.dice_weight * dice_loss_value + self.ssim_weight * ssim_loss_value
...
if __name__ == '__main__':
    ...
    # criterion = SSIMLoss()
    criterion = CombinedLoss(dice_weight=0.85, ssim_weight=0.15)
